...
void run() {
    ...
    vector<mod_int> ways(n+1, 0); // partial sum of ways
    vector<mod_int> vals(n+1, 0); // partial sum of vals
    ...
    if (count[0] == 0) {
        ...
        for (int i=count[0]; i > 0; i--) {
            ways[i - 1] = combo.choose(count[0], i) + ways[i];
            vals[i - 1] = combo.choose(count[0], i)*i + vals[i];
        }
    }

    bool remaining = true;
    if (count[1] == 0) remaining = false;
    int index = count[0];
    int num = 1;
    while (remaining) {
        int c = count[num];
        if (c > count[num - 1]) {
            c = count[num - 1];
        } else if (c < count[num - 1]) {
            for (int i = count[num - 1] - c; i < count[num - 1]; i++)
            ways[index + c] += ways[index - count[num-1] + i - 1] * combo.choose(count[num], i);
        }

        for (int i=c; i > 0; i--) {
            ways[index + i - 1] = ways[index - count[num - 1] + i - 1] * combo.choose(count[num], i) + ways[index + i];
        }
        
        for (int i=c; i > 0; i--) {
            vals[index + i - 1] = (vals[index - count[num - 1] + i - 1] + ways[index - count[num - 1] + i - 1] * c);
        }

        //for (int i=count[num]; i > 0; i--) {
        //    ways[index + i - 1] = ways[index - count[num - 1] + count[num] - i] * combo.choose(count[1], i) + ways[index + i];
        //}

        index += count[num];
        num++;
        remaining = count[num] >= 1;
    }
